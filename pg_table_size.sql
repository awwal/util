...
JOIN pg_stat_user_tables AS stats USING (relname)
...
FROM pg_catalog.pg_statio_user_tables AS statsio
JOIN pg_stat_user_tables AS stats USING (relname) WHERE stats.schemaname = current_schema

ORDER BY TBL, live_rows ASC;
